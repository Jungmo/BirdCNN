...
#define ORIGINAL_SIZE 200
#define RESIZED_SIZE 150
...
int resizeBilinearGray(unsigned char image[41078], unsigned char ret[RESIZED_SIZE * RESIZED_SIZE],  int w, int h, int w2, int h2)
{
	int A, B, C, D, x, y, index, gray;
	float x_ratio = ((float)(w - 1)) / w2;
	float y_ratio = ((float)(h - 1)) / h2;
